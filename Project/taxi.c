...
int main(void){
    ...
    pointer_map_sh->MAPPA[coordinate_taxi.x][coordinate_taxi.y].num_taxi_presenti++;

    pointer_map_sh->MAPPA[coordinate_taxi.x][coordinate_taxi.y].symbol_in_map = 'T';
    ...
    reserveSem(sem_wait_childs); //Decrementa il semaforo sem_wait_childs dato che ha finito la sincronizzazione ed è pronto a comunicare

    while (semctl(sem_wait_childs,0,GETVAL)!=0);
    ...
    while (semctl(sem_simulation, 0, GETVAL) == 1 ) { // AVVIO COMMUNICATION DEI PROCESSI TAXI - POSSONO INIZIARE A SODDISFARE LE RICHIESTE !!
        ...
        if (msgrcv(queue, &messaggio, sizeof(messaggio), getpid(), IPC_NOWAIT) != -1 ) { //Se c'è un messaggio
            ...
            if (sscanf(messaggio.mtext, "%d %d %d %d %d", &pid_richiesta, &x_richiesta, &y_richiesta, &x_destinazione,
                       &y_destinazione) == 5) {

                printf(BOLDRED"\nSono il processo Taxi : %d ---- RICHIESTA Trovata - PID Processo Richiesta : %d  - CellaRichiesta [%d-%d]  - Destinazione [%d-%d]\n"RESET,getpid(), pid_richiesta, x_richiesta, y_richiesta, x_destinazione,y_destinazione);
            ...
            if (status == 1 && pointer_map_sh->taxi_array[index].bloccato == 0) {
                ...
                if (x_taxi == x_destinazione && y_taxi == y_destinazione) {
                    ...
                    if (pointer_map_sh->taxi_array[index].distance_counter >
                        pointer_map_sh->taxi_max_cell_counter.distance_counter) {

                        pointer_map_sh->taxi_max_cell_counter = pointer_map_sh->taxi_array[index];

                    }

                    if (pointer_map_sh->taxi_array[index].max_time_request >
                        pointer_map_sh->taxi_max_time.max_time_request) {

                        pointer_map_sh->taxi_max_time = pointer_map_sh->taxi_array[index];

                    }

                    if (pointer_map_sh->taxi_array[index].request_counter >
